...
} from './../../../../../../open-api-modules/customer-api-docs/model/apiResponseTNGResponse';
...
} from './../../../../../../open-api-modules/customer-api-docs/model/additionalEmployeeDataRequest';
...
@Component({
  selector: 'app-tng-confirm-infomation',
  templateUrl: './tng-confirm-infomation.component.html',
  styleUrls: ['./tng-confirm-infomation.component.scss'],
})
export class TngConfirmInfomationComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  displayCompanyInfo: boolean = false;
  ...
  initConfirmInfoFormData() {
    this.infoForm.patchValue({
      ...
      //   : [''],
      // commune: this.customerInfo.personalData?.communeId
      //   ? this.customerInfo.personalData?.communeId
      //   : [''],
  ...
  initCompanyInfoFormData() {
    this.companyInfoForm.patchValue({
      employeeCode: this.customerInfo.personalData.organizationName,
    });
    if (this.customerInfo.personalData.organizationName) {
      this.companyInfoForm.disable();
    } else {
      this.companyInfoForm.controls.employeeCode.setValue('TNG');
    }
  }
  ...
  onCompanyInfoSubmit() {
    //Check Tng employee code
    this.tngControllerService
      .isEmployeesWorkingAtTNG({
        employeeCode: this.companyInfoForm.controls.employeeCode.value,
      })
      .subscribe((result: ApiResponseTNGResponse) => {
        if (!result || result.responseCode !== 200) {
          return this.handleResponseError(result.errorCode);
        }
        if (result.result.isEmployee === 'false') {
          ...
          return this.companyInfoForm.controls.employeeCode.enable();
        }
        //call Api submit employee code
        return this.submitEmployeeData();
      });
  }

  submitEmployeeData() {
    const additionalEmployeeDataRequest: AdditionalEmployeeDataRequest = {
      organizationName: this.companyInfoForm.controls.employeeCode.value,
    };
    this.infoControllerService
      .addEmployeeData(additionalEmployeeDataRequest)
      .subscribe((result) => {
        if (!result || result.responseCode !== 200) {
          return this.handleResponseError(result.errorCode);
        }
