...
@Component({
  selector: 'app-vac-sign-contract',
  templateUrl: './vac-sign-contract.component.html',
  styleUrls: ['./vac-sign-contract.component.scss'],
})
export class VacSignContractComponent implements OnInit {
    ...
    this.subManager.add(
      this.isSignContractSuccess$.subscribe((isSignContractSuccess) => {
        ...
        if (this.isSignContractSuccess) {
          return this.router.navigateByUrl('/vac/sign-contract-success');
        }
      })
    );

    if (this.isSignContractSuccess) {
      this.router.navigateByUrl('/vac/sign-contract-success');
    }
  ...
  get showSignContractBtn() {
    if (this.currentLoan.termType === PAYDAY_LOAN_TERM_TYPE_VAC.ONE_MONTH) {
      return (
        (this.contractInfo &&
  ...
  getActiveLoan() {
    const applicationType =
      this.userInfo.personalData.employeeType ===
    ...
    return this.applicationControllerService
      .getActivePaydayLoan(this.coreToken, applicationType)
      .pipe(
  ...
  acceptContract() {
    return this.borrowerControllerService
      .acceptContract({
        ...
        contractId:
          this.currentLoan.termType === PAYDAY_LOAN_TERM_TYPE_VAC.ONE_MONTH
            ? environment.CONTRACT_VAC_UUID
            : environment.CONTRACT_VAC_UUID_THREE_MONTHS,
  ...
  sendContractOtp(params: SendContractOTPRequest) {
    ...
    return this.contractControllerService
      .sendContract(companyName, params)
      .pipe(
